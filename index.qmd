...
Air is thinner at higher altitudes. When air is thinner, there is less resistance acting on a baseball when it is hit, so the ball can travel farther. That can lead to more extra-base hits and home runs, which increases total runs.

Coors Field is located in Denver, which is at a much higher altitude than most MLB stadiums. Because of this, we expect Coors Field to have higher average total runs and home runs per game compared to other stadiums.
...
The dataset contains 2,430 games in 2010 and 2,429 games in 2021. Each row represents a single game played during that season.

We aggregate individual games into stadium averages because our goal is to compare ballparks, not individual matchups. Single games can vary due to randomness, weather, or team performance. By calculating average total runs and average home runs per stadium, we reduce noise and create a fair and stable comparison between stadiums.

This aggregation step prepares the data for visualization by giving us one clear value per stadium, which makes comparisons meaningful and easier to interpret.
...
A horizontal bar chart is better than a scatter plot because stadium names are categorical labels. Scatter plots are more appropriate for showing relationships between two numeric variables, while bar charts are designed for comparing values across categories. With many stadiums, horizontal bars make labels easier to read and comparisons clearer.

A vertical bar chart would be useful if there were fewer categories or shorter labels. However, with many stadiums, horizontal bars improve readability and make ranking easier to see.

The color highlighting draws attention specifically to Colorado while still showing all other stadiums for context. This helps the audience immediately identify the key comparison without removing the full league perspective.

Position (the length of the bars) communicates numerical differences more accurately than color. Color is mainly used for emphasis and grouping, while position is the primary way we encode magnitude.
...
This visualization is suitable for a business presentation because it includes a clear and direct title that states the conclusion, properly labeled axes with units, and sorted bars for easy comparison. The legend clearly explains the color highlighting, and the clean background with subtle gridlines improves readability without adding distraction.

The annotation is helpful because it highlights Colorado’s exact value, making the key takeaway easier to understand without requiring the audience to estimate visually. Its placement was adjusted to align precisely with the Colorado bar.
...
# Sort stadiums by 2010 average runs for consistent ordering
comparison_data_sorted = comparison_data.sort_values("runs_2010", ascending=True)

# Set style for consistency with earlier plots
plt.style.use("seaborn-v0_8-whitegrid")

# Create side-by-side horizontal bar charts sharing the y-axis
fig, (ax2010, ax2021) = plt.subplots(
    nrows=1,
    ncols=2,
    figsize=(12, 8),
    sharey=True
...
# Colors highlighting Colorado
colors_2010 = ["darkorchid" if team == "COL" else "lightgrey" for team in comparison_data_sorted["home"]]
colors_2021 = ["darkorchid" if team == "COL" else "lightgrey" for team in comparison_data_sorted["home"]]

# 2010 horizontal bar chart
ax2010.barh(
    y=comparison_data_sorted["home"],
    width=comparison_data_sorted["runs_2010"],
    color=colors_2010
)
ax2010.set_title("Average Runs per Game by Stadium (2010)", fontsize=12, fontweight="bold")
ax2010.set_xlabel("Average Runs per Game")
ax2010.set_ylabel("Stadium (Home Team)")

# 2021 horizontal bar chart (same stadium order, shared y-axis)
ax2021.barh(
    y=comparison_data_sorted["home"],
    width=comparison_data_sorted["runs_2021"],
    color=colors_2021
)
ax2021.set_title("Average Runs per Game by Stadium (2021)", fontsize=12, fontweight="bold")
ax2021.set_xlabel("Average Runs per Game")

# Highlight Colorado in the y-axis labels by making its label bold and purple
for ax in (ax2010, ax2021):
    ax.tick_params(axis="y", labelsize=9)
    labels = ax.get_yticklabels()
    for label in labels:
        if label.get_text() == "COL":
            label.set_color("darkorchid")
            label.set_fontweight("bold")

# Add a shared main title
fig.suptitle("Run-Friendliness of MLB Stadiums: 2010 vs 2021", fontsize=16, fontweight="bold", y=0.95)
...
This two-panel visualization compares average runs per game across MLB stadiums in 2010 and 2021 while keeping the stadium order fixed based on 2010 values. Maintaining the same order allows for easier comparison across seasons without reinterpreting rankings. Colorado (COL) is highlighted to clearly show how Coors Field performs relative to the rest of the league in both years.

If Colorado remains near the top in both panels, it supports the claim that Coors Field is consistently run-friendly. If its position changes in 2021, the conclusion becomes more nuanced, suggesting that run environments can shift over time due to broader league factors. Overall, this visualization strengthens the analysis by adding a time comparison rather than relying on a single season.
